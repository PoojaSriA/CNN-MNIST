...
def plot_count_per_class(yd):
    #ydf = pd.DataFrame(yd)
    ydf = pd.DataFrame(yd, columns=['Category'])
    f, ax = plt.subplots(1,1, figsize=(12,4))
    #g = sns.countplot(ydf[0], order = np.arange(0,10))
    g = sns.countplot(x='Category', data=ydf, order=np.arange(0,10))
...
new_working_directory = 'C:/Users/Pooja/data/'
os.chdir(new_working_directory)
print("Current working directory:", os.getcwd())

y_cat = model.predict(X_test, batch_size=64)
print("Prediction shape:", y_cat.shape) 
